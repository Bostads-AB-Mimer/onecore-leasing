...
} from '../adapters/xpand/tenant-lease-adapter-v2'
...
export const routes = (router: KoaRouter) => {
  /**
   * @swagger
   * /v2/leases/for/nationalRegistrationNumber/{pnr}:
   *   get:
  ...
  /**
   * @swagger
   * /v2/leases/for/contactCode/{contactCode}:
   *   get:
  ...
  router.get('(.*)/v2/leases/for/contactCode/:pnr', async (ctx) => {
    const metadata = generateRouteMetadata(ctx, [
      'includeTerminatedLeases',
      'includeContacts',
    ])
  ...
  /**
   * @swagger
   * /v2/leases/for/propertyId/{propertyId}:
   *   get:
  ...
  router.get('(.*)/v2/leases/for/propertyId/:propertyId', async (ctx) => {
    const metadata = generateRouteMetadata(ctx, [
      'includeTerminatedLeases',
      'includeContacts',
    ])
   ...
   */
  router.get('(.*)/v2/leases/:id', async (ctx) => {
   ...
   */
  router.post('(.*)/v2/leases', async (ctx) => {
    const metadata = generateRouteMetadata(ctx)
    try {
